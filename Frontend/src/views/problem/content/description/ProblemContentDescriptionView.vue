  ...
  <ContentBaseSecondary>
    <div class="row">
      <div class="col-sm-9 col-xs-12" id="content">
        <h4>题目描述</h4>
        <div key="1" v-html="problem_content.description"></div>
        <h4>输入格式</h4>
      ...
      <div
        ...
        style="padding-left: 0; top: -10px; margin-top: 10px"
      >
            ...
            <tbody>
              <tr>
                <td style="padding: 15px 8px 15px 8px">
                  难度：
                  <el-tag style="float: right" class="ml-2" :type="tag">{{
                    problem_content.difficulty
                  }}</el-tag>
                </td>
              </tr>
              <tr>
                <td style="padding: 15px 8px 15px 8px">
                  ...
                  <span style="float: right">{{
                    problem_content.time_limit +
                    "s/" +
                    problem_content.space_limit +
                    "M"
                  }}</span>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding: 15px 8px 15px 8px">
                  <span class="problem-algorithm-tag" style="cursor: pointer"
                    >算法标签<span>
                      <svg
                        @click="showAlgoTag"
                    ...
                    <div
                      v-show="showTag"
                      class="problem-algorithm-tag-field"
                      style=""
                    >
                      <span
                        v-for="tag in problem_content.tags.split(/\s+/)"
                        :key="tag.id"
                        class="problem-algorithm-tag-field-item"
                        >{{ tag }}</span
                      >
                    </div>
                  </span>
      ...
      $.ajax({
        ...
        success(res) {
          if (res.result === "success") {
            ...
            if (type === "debug") {
              editor_child.value.changeDebugOutput(
                res["status"],
                res["result"],
                res["real_time"]
              );
