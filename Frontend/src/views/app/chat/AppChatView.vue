    ...
    <ContentBase>
        <div class="row">
            <div class="col-md-3 col-sm-12" >
                ...
                <div @click="select(item.id, item.name)" :class="{ select: item.id === select_id }"
                    class="row friend" style="height:60px;display:flex;align-items:center;
                    border-bottom: 1px solid #E4E7ED;" v-for="(item, index) in friends" :key="index">
                    <div class="col-2">
                ...
                <div class="u-chat">
                    <div  class="chat-container translate active">
                        ...
                        <div v-if="select_id !== 0" id="chat-footer" class="footer">
                            <!-- 文字输入 -->
                            <el-input resize="none" id="emojiInput" maxlength="1024" show-word-limit v-model="content" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }"
                                placeholder="请输入内容" ></el-input>
                            <el-button  @click="submit" type="primary" round style="margin-left: 5px;">
                                发送
                                <u-icon size="16" class="select-none cursor-pointer" style="padding-bottom: 5px">
...
export default {
    ...
    setup() { 
        const store = useStore();
        let content = ref(''), select_id = ref(0), select_name = ref('');
        let socket = new WebSocket('wss://app2105.acapp.acwing.com.cn/wss/chat/?token=' + store.state.user.access);
        let messageRef = ref();
        let data = reactive([]), friends = reactive([]), user = reactive({
            id: store.state.user.id,
            photo: store.state.user.photo,
            name: store.state.user.name,
        });
        store.state.user.socket = new WebSocket('wss://app2105.acapp.acwing.com.cn/wss/chat/list/?token=' + store.state.user.access);
        ...
        const select = (id, name) => { 
            ...
            if (id !== select_id.value) {
                ...
                socket = new WebSocket('wss://app2105.acapp.acwing.com.cn/wss/chat/?token=' + store.state.user.access);
                socket.onopen = () => { 
                    socket.send(JSON.stringify({
                        'event': 'init',
                        'target': parseInt(id),
